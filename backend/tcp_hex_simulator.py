...
compliant with a simplified JT/T 794-style layout and POSTs it to the CoolPet
ingestion endpoint. In production this would be a raw TCP socket; here we use
the containerized HTTP/WebSocket bridge.

Usage:
    python tcp_hex_simulator.py <IMEI_HEX> [--rounds 100] [--interval 2]
...
def post_hex(url: str, hex_str: str) -> dict:
    data = json.dumps({"hex": hex_str}).encode()
...
def main():
    ...
    bpm = 92
    battery = 96
    print(f"[coolpet-collar] sending to {args.url} — imei={args.imei_hex}")
